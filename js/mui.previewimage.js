(function($, window) {
	...
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			...
			zoom: true,
			header: '',
			footer: '<span class="mui-preview-indicator img-num"></span><a id="downloadImg">保存</a>'
		}, options || {});
		this.init();
		this.initEvent();
	};
	...
	proto.init = function() {
		...
		this.download = this.element.querySelector($.classSelector('.downloadImg'));
		if(options.footer) {
		...
		if(document.getElementById("downloadImg")){
					document.getElementById("downloadImg").addEventListener('tap', function(e) {
			var a = e.srcElement.parentElement.parentElement.getElementsByClassName('mui-slider-item mui-zoom-wrapper mui-active')[0];
			var imgUrl = a.getElementsByTagName('img')[0].src;
			if(mui.os.plus) {
				mui.plusReady(function() {
					var waiting = showWaiting();
					var downLoader = plus.downloader.createDownload(imgUrl, {
						method: 'GET',
						filename: '_downloads/image/' + cutImageSuffix(imgUrl)
	...
	proto.initEvent = function() {
		...
		this.element.addEventListener('slide', function(e) {
			...
			self.download = e.srcElement.querySelectorAll('.mui-zoom')[slideNumber];
