	...
	$.fn.zyUpload = function(options, param) {
		...
		return this.each(function() {
			...
			this.funDisposePreviewHtml = function(file, e) {
				//mui.toast('预览的html'+(JSON.stringify(file)))
				var html = "";
				var imgWidth = parseInt(para.itemWidth.replace("px", "")) - 15;
					...
					onSelect: function(selectFiles, allFiles) {
						...
						var funDealtPreviewHtml = function() {
							...
							if(file) {
								...
								touchNames.push(file.name);
								reader.onload = function(e) {
									zyUpload.webFiles.push(e.target.result)
									if(oldImageAttachments.length==0){
										document.getElementById("imgs").innerHTML = "";
									}
									if(i==selectFiles.length-1){
										document.getElementById("imgs").appendChild(creatWebImg(zyUpload.webFiles))
									}
									// 处理下配置参数和格式的html
...
//									// 再接着调用此方法递归组成可以预览的html
					...
					onDelete: function(file, files) {
						...
						console.info("剩下的文件");
						console.info(files);
						//mui.toast("剩下的文件"+(JSON.stringify(files)))
					},
					...
					onSuccess: function(file, response) {
						attachmentIds.push(JSON.parse(response).Data.AttachmentIds);
